...
description: Patterns for invoking the GitHub CLI (gh) from agents. Covers structured output, pagination, repo targeting, search vs list, gh api fallback. Use when the agent needs to interact with GitHub via gh CLI or when gh command output needs parsing.
allowed-tools: gh, bash
metadata:
  tags: "gh github-cli json pagination api fallback agent-patterns"
---
