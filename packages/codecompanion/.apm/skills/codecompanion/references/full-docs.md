...
> CodeCompanion.nvim is an AI coding plugin for Neovim. Chat with Claude, GPT-5, and Gemini, run CLI agents like Claude Code and Codex, and edit code inline.
...
- [Features](#features)
- [Overview](#overview)
- [Supported LLMs and Agents](#supported-llms-and-agents)
...
- [Setup](#setup)
  - [Chat and Inline](#chat-and-inline)
  - [CLI](#cli)
- [Usage](#usage)
  - [Chat](#chat)
  - [CLI](#cli)
...
- [Suggested Plugin Workflow](#suggested-plugin-workflow)
- [How Context Is Managed](#how-context-is-managed)
  - [In the Chat Buffer](#in-the-chat-buffer)
...
- [Protocol Version](#protocol-version)
- [See Also](#see-also)
- [v18.7.0 to v19.0.0](#v1870-to-v1900)
  - [Adapters](#adapters)
  - [Config](#config)
  - [Prompt Library](#prompt-library)
- [v17.33.0 to v18.0.0](#v17330-to-v1800)
  - [Config](#config)
  - [Adapters](#adapters)
  - [Chat](#chat)
  - [Prompt Library](#prompt-library)
  ...
  - [UI](#ui)
- [Layout](#layout)
- [Changing the Default Adapter](#changing-the-default-adapter)
- [Changing the Default Model](#changing-the-default-model)
- [Changing Adapter Parameters (Schema)](#changing-adapter-parameters-schema)
- [Adding a Custom Adapter](#adding-a-custom-adapter)
- [Controlling Model Choices](#controlling-model-choices)
- [Environment Variables](#environment-variables)
- [Disabling Compaction](#disabling-compaction)
- [Hiding Preset Adapters](#hiding-preset-adapters)
- [Setting a Proxy](#setting-a-proxy)
- [Setup Examples](#setup-examples)
  - [Azure OpenAI](#azure-openai)
  - [llama.cpp with `--reasoning-format deepseek`](#llamacpp-with---reasoning-format-deepseek)
  - [Ollama (remotely)](#ollama-remotely)
  - [OpenAI Responses API](#openai-responses-api)
- [Community Adapters](#community-adapters)
- [Configuring Adapter Settings](#configuring-adapter-settings)
- [Setting a Default Adapter](#setting-a-default-adapter)
- [Setting Default Session Config Options](#setting-default-session-config-options)
  - [Models](#models)
  - [Others](#others)
- [Configuring MCP Servers](#configuring-mcp-servers)
- [Hiding Preset Adapters](#hiding-preset-adapters)
- [Setup: Auggie CLI from Augment Code](#setup-auggie-cli-from-augment-code)
- [Setup: Cagent](#setup-cagent)
- [Setup: Claude Code](#setup-claude-code)
  - [Using Claude Pro Subscription](#using-claude-pro-subscription)
  - [Using an API Key](#using-an-api-key)
- [Setup: Cline CLI](#setup-cline-cli)
- [Setup: Codex](#setup-codex)
- [Setup: Copilot CLI](#setup-copilot-cli)
- [Setup: Cursor CLI](#setup-cursor-cli)
- [Setup: Gemini CLI](#setup-gemini-cli)
- [Setup: Goose CLI](#setup-goose-cli)
- [Setup: Kilo Code](#setup-kilo-code)
- [Setup: Kimi CLI](#setup-kimi-cli)
- [Setup: Kiro CLI](#setup-kiro-cli)
- [Setup: Mistral Vibe](#setup-mistral-vibe)
- [Setup: OpenCode](#setup-opencode)
- [Creating Custom ACP Adapters](#creating-custom-acp-adapters)
- [Changing Adapter](#changing-adapter)
- [Completion](#completion)
  - [Prefixes](#prefixes)
- [Callbacks](#callbacks)
  - [Available Events](#available-events)
  - [Registering Callbacks](#registering-callbacks)
  - [Background Callbacks](#background-callbacks)
  - [Preventing Submission](#preventing-submission)
  - [Truncating Tool Output](#truncating-tool-output)
...
- [Tools](#tools)
  ...
  - [Approvals](#approvals)
  - [Auto Submit (Recursion)](#auto-submit-recursion)
...
- [Overriding Tool Behaviour](#overriding-tool-behaviour)
  - [Tool Defaults](#tool-defaults)
  - [Override Options](#override-options)
- [Adding Prompts](#adding-prompts)
  - [Refreshing Markdown Prompts](#refreshing-markdown-prompts)
...
- [user](#user)
    - [External Lua Files](#external-lua-files)
- [user](#user)
- [user](#user)
...
- [user](#user)
- [Others](#others)
  - [Hiding Built-in Prompts](#hiding-built-in-prompts)
- [Enabling Rules](#enabling-rules)
- [Rule Groups](#rule-groups)
  - [Autoload](#autoload)
- [Parsers](#parsers)
  - [Applying Parsers](#applying-parsers)
- [Chat System Prompt](#chat-system-prompt)
- [Tool System Prompt](#tool-system-prompt)
- [Changing System Prompts](#changing-system-prompts)
  - [Chat](#chat)
  - [Tools](#tools)
- [When System Prompts Change](#when-system-prompts-change)
- [Language](#language)
- [Log Level](#log-level)
- [Per-Project Configuration](#per-project-configuration)
- [Sending Code](#sending-code)
- [Copying code from a chat buffer](#copying-code-from-a-chat-buffer)
- [Applying an LLM's edits to a buffer or file](#applying-an-llms-edits-to-a-buffer-or-file)
- [Run tests from the chat buffer](#run-tests-from-the-chat-buffer)
- [Navigating between responses in the chat buffer](#navigating-between-responses-in-the-chat-buffer)
- [Quickly accessing a chat buffer](#quickly-accessing-a-chat-buffer)
- [Built-in Prompts](#built-in-prompts)
- [Action Palette](#action-palette)
- [Changing Adapter and Model](#changing-adapter-and-model)
- [Changing ACP Command](#changing-acp-command)
- [Completion](#completion)
- [Context](#context)
...
- [Agents / Tool Groups](#agents-tool-groups)
  - [Turning a group into an agent](#turning-a-group-into-an-agent)
  - [agent](#agent)
  - [files](#files)
...
- [#buffer](#buffer)
  - [Basic Usage](#basic-usage)
  - [Target Specific Buffers](#target-specific-buffers)
...
- [#terminal](#terminal)
- [#viewport](#viewport)
- [Default Rule Group](#default-rule-group)
- [Creating Rules](#creating-rules)
  - [Example 1: Rule that can be processed with the `codecompanion` parser](#example-1-rule-that-can-be-processed-with-the-codecompanion-parser)
- [System Prompt](#system-prompt)
- [My other header](#my-other-header)
  - [Example 2: Rule that can be processed with the `claude` parser](#example-2-rule-that-can-be-processed-with-the-claude-parser)
- [Adding Rules to a Chat Buffer](#adding-rules-to-a-chat-buffer)
  - [When Opening the Chat Buffer](#when-opening-the-chat-buffer)
  - [Slash Command](#slash-command)
  - [Action Palette](#action-palette)
  - [Clearing Rules](#clearing-rules)
- [/acp_session_options](#acp-session-options)
- [/buffer](#buffer)
- [/command](#command)
- [/compact](#compact)
- [/fetch](#fetch)
...
- [/resume](#resume)
- [/symbols](#symbols)
- [Initiating a CLI Interaction](#initiating-a-cli-interaction)
- [Workflow](#workflow)
  - [Prompting the Agent](#prompting-the-agent)
  - [Adding Context](#adding-context)
  - [Fixing LSP Diagnostics](#fixing-lsp-diagnostics)
...
- [Handlers](#handlers)
  ...
  - [OpenAI's API Output](#openais-api-output)
  - [Chat Buffer Output](#chat-buffer-output)
- [Me](#me)
  - [`request.build_messages`](#requestbuild-messages)
  - [`response.parse_chat`](#responseparse-chat)
  - [`response.parse_meta`](#responseparse-meta)
  - [`request.build_parameters`](#requestbuild-parameters)
  - [`response.parse_inline`](#responseparse-inline)
  - [`lifecycle.on_exit`](#lifecycleon-exit)
  - [`lifecycle.setup` and `lifecycle.teardown`](#lifecyclesetup-and-lifecycleteardown)
  - [The Utility File](#the-utility-file)
- [Schema](#schema)
- [Function Calling / Tool Use](#function-calling-tool-use)
- [Migrating from Old Handler Format](#migrating-from-old-handler-format)
- [How They Work](#how-they-work)
- [Creating Agentic Workflows](#creating-agentic-workflows)
  - [Steps to Follow](#steps-to-follow)
  - [Steps to Follow](#steps-to-follow)
- [Using Extensions](#using-extensions)
- [Creating Extensions](#creating-extensions)
  - [Extending Chat Functionality](#extending-chat-functionality)
- [Local Extensions](#local-extensions)
- [Dynamic registration](#dynamic-registration)
- [Best Practices](#best-practices)
- [Structure of a Parser](#structure-of-a-parser)
...
- [CONTEXT](#context)
  ...
  - [`handlers`](#handlers)
  - [`output`](#output)
  - [Running the Calculator tool](#running-the-calculator-tool)
- [CONTEXT](#context)
  - [OBJECTIVE](#objective)
  - [RESPONSE](#response)
  - [Adding in User Approvals](#adding-in-user-approvals)
- [Extending from the run_command tool](#extending-from-the-run-command-tool)
- [Supporting an Adapter Tool](#supporting-an-adapter-tool)
- [Other Tips](#other-tips)
  - [`use_handlers_once`](#use-handlers-once)
- [Progress updates with Fidget.nvim by [@jessevdp](https://github.com/jessevdp)](#progress-updates-with-fidgetnvim-by-jessevdp)
- [Inline spinner with Fidget.nvim by [@yuhua99](https://github.com/yuhua99)](#inline-spinner-with-fidgetnvim-by-yuhua99)
- [Status column extmarks with the inline interaction by [@lucobellic](https://github.com/lucobellic)](#status-column-extmarks-with-the-inline-interaction-by-lucobellic)
- [Lualine.nvim integration](#lualinenvim-integration)
- [Heirline.nvim integration](#heirlinenvim-integration)
...
- :heart_hands: User contributed and supported [adapters](#http-adapters-configuration-community-adapters)
- :battery: Support for [Model Context Protocol (MCP)](#model-context-protocol-mcp)
- :rocket: [Inline transformations](#using-inline), code creation and refactoring
- :robot: [Editor Context](#editor-context), [Slash Commands](#slash-commands), [Tools](#agents-and-tools) and [Workflows](#workflows) to improve LLM output
- :brain: Support for [rules](#using-rules) files like `CLAUDE.md`, `.cursor/rules` and your own custom ones
- :sparkles: Built-in [prompt library](#using-the-action-palette) for common tasks like advice on LSP errors and code explanations
- :building_construction: Create your own [custom prompts](#configurationprompt-librarycreating-prompts), Editor Context and Slash Commands
- :books: Have [multiple chats](#usage-tips-quickly-accessing-a-chat-buffer) open at the same time
- :art: Support for [vision and images](#using-the-chat-buffer-images-vision) as input
...
The plugin allows you to specify adapters for each interaction and also for each [prompt library](#configurationprompt-library) entry.

## Supported LLMs and Agents

CodeCompanion uses [HTTP](#configurationadapters-http) and [ACP](#configurationadapters-acp) adapters to connect to LLMs and agents. Out of the box, the plugin supports:
...
In order to add a custom adapter, please refer to the [extending adapters](#extending-with-adapters) documentation. Also, be sure to check out the [community adapters](#configurationadapters-httpcommunity-adapters) section for user contributed adapters.


---

> Install CodeCompanion.nvim with lazy.nvim or packer. Covers dependencies, Neovim 0.11+ requirements, optional integrations like nvim-cmp, and API key setup.
...
[nvim-treesitter](https://github.com/nvim-treesitter/nvim-treesitter) is required if you plan to use markdown prompts in the [prompt library](#prompt-library-configuration), ensuring you have the `yaml` parser installed.
...
Visit the [extensions documentation](#extendingextensions) to learn more about available extensions and how to create your own.
...
When in the [chat buffer](#using-the-chat-buffer), completion can be used to more easily add [editor context](#editor-context), [slash commands](#slash-commands) and [tools](#agents-and-tools). Out of the box, the plugin supports completion with both [nvim-cmp](https://github.com/hrsh7th/nvim-cmp) and [blink.cmp](https://github.com/Saghen/blink.cmp). For the latter, on version <= 0.10.0, ensure that you've added `codecompanion` as a source:
...
The plugin also supports [native completion](#using-the-chat-buffer-completion) and [coc.nvim](https://github.com/neoclide/coc.nvim).
...
> Get up and running with CodeCompanion in Neovim — configure your first LLM adapter, open a chat buffer, use the inline interaction, and learn the core commands.
...
> [community-contributed adapters](#configurationadapters-httpcommunity-adapters).
>
> [ACP adapters](#acp-adapters-configuration) are only supported for the chat interaction.
...
The [chat](#using-the-chat-buffer) and [inline](#using-inline) interactions need an adapter to function. In CodeCompanion terminology, an adapter is the connection between Neovim and an LLM or agent. CodeCompanion has two _types_ of adapters; HTTP adapters which connect you to an LLM via it's API and ACP adapters which connect you to an agent via the [Agent Client Protocol](https://agentclientprotocol.com). CodeCompanion has a number of [built-in adapters](https://github.com/olimorris/codecompanion.nvim/blob/main/lua/codecompanion/config.lua) that you can leverage and you can find more details in the respective [HTTP](#http-adapters-configuration) and [ACP](#acp-adapters-configuration) sections of the documentation.
...
There are numerous ways that environment variables can be set for adapters. Refer to the [environment variables](#http-adapters-configuration-environment-variables) section for more information.
...
_Editor Context_, accessed via `#` (by default), contain data about the present state of Neovim. You can find a [list of available editor context](#editor-context). The buffer editor context will automatically link a buffer to the chat buffer, by default, updating the LLM when the buffer changes.
...
_Slash commands_, accessed via `/` (by default), run commands to insert additional context into the chat buffer. You can find a [list of available slash commands and how to use them](#slash-commands).

**Tools**

_Tools_, accessed via `@` (by default), allow the LLM to function as an agent and leverage external tools. You can find a [list of available tools and how to use them](#usagechat-bufferagents-toolsavailable-tools).
...
Run `:CodeCompanion your prompt` to call the inline interaction. The interaction will evaluate the prompt and either write code or open a chat buffer. You can also make a visual selection and call the inline interaction. To send additional context alongside your prompt, you can leverage [editor context](#using-inline-editor-context) such as `:CodeCompanion #{buffer} <your prompt>`.
...
Run `:CodeCompanionActions` to open the action palette, which gives you access to the plugin's features, including your prompts from the [prompt library](#prompt-library-configuration).
...
- `CodeCompanion /<prompt library>` - Call an item via its alias from the [prompt library](#configurationprompt-library)
...
> **NOTE:** 
> You can also assign prompts from the library to specific mappings. See the [prompt library](#configurationprompt-libraryassigning-prompts-to-a-keymap) section for more information.


---

> How CodeCompanion manages LLM context windows, handles token limits, and is architected internally — reference for contributors and advanced users.
...
Firstly, CodeCompanion manages context by paying close attention to the number of tokens in the [chat buffer](#using-the-chat-buffer), matching them against a defined trigger threshold in your config, which can be [customised](#chat-buffer-configuration-context-management).


---

> CodeCompanion's Agent Client Protocol (ACP) support — covers session management, tool execution, permissions, and which capabilities are currently implemented.
...
Please see the [Configuring ACP Adapters](#acp-adapters-configuration) page.
...
- [Configuring ACP Adapters](#acp-adapters-configuration) - Setup instructions for specific agents
- [Using Agents and Tools](#agents-and-tools) - How to interact with agents in chat


---

> Overview of CodeCompanion's Model Context Protocol (MCP) support — which capabilities are implemented and how MCP tools appear in the chat buffer in Neovim.
...
To use MCP servers within CodeCompanion, refer to the [tools](#agents-and-tools-mcp) section in the chat buffer usage section of the documentation.

If [enabled](#mcp-servers-configuration-enabling-servers), the servers will be started when you open a chat buffer for the first time. However, you can use the [MCP slash command](#slash-commands-mcp) to start or stop servers manually.
...
> Step-by-step guide for upgrading CodeCompanion between major versions, covering breaking changes, configuration migration, and version pinning in Neovim.
...
- The `full_stack_dev` group has been renamed to [agent](#agents-and-tools-agent) ([#2786](https://github.com/olimorris/codecompanion.nvim/pull/2786))
...
- Workspaces have now been removed from the plugin. Please use [Rules](#rules-configuration) instead.
...
- Memory has been renamed to rules. Please rename any references to `memory` in your configuration to `rules`. Please refer to the [Rules](#rules-configuration) documentation for more information ([#2440](https://github.com/olimorris/codecompanion.nvim/pull/2440))
...
> Configure the CodeCompanion Action Palette — your entry point to chat buffers, prompt library prompts, and plugin features in Neovim.
...
> Configure CodeCompanion's HTTP adapters to connect Neovim to OpenAI, Anthropic, Copilot, Gemini, and Ollama. Covers API keys, model selection, and proxy settings.


# HTTP Adapters Configuration

> **TIP:** 
> Want to connect to an LLM that isn't supported out of the box? Check out
> [these](#community-adapters) user contributed adapters, [create](#extending-with-adapters) your own or post in the [discussions](https://github.com/olimorris/codecompanion.nvim/discussions)
...
There are two "types" of adapter in CodeCompanion; **http** adapters which connect you to an LLM and [ACP](#acp-adapters-configuration) adapters which leverage the [Agent Client Protocol](https://agentclientprotocol.com) to connect you to an agent.
...
> **NOTE:** 
> See the [Creating Adapters](#extending-with-adapters) section to learn how to create custom adapters
...
> Configure Agent Client Protocol (ACP) adapters in CodeCompanion to connect with CLI agents like Claude Code, Codex, Gemini CLI, and OpenCode from Neovim.


# ACP Adapters Configuration

This section contains configuration which is specific to Agent Client Protocol (ACP) adapters only. There is a lot of shared functionality between ACP and [http](#http-adapters-configuration) adapters. Therefore it's recommended you read the two pages together.
...
To find out what the available session config options are for a specific adapter you can open the [debug window](#using-the-chat-buffer-debug-window) in the chat buffer.

## Configuring MCP Servers

Some ACP adapters [support](https://agentclientprotocol.com/protocol/session-setup#mcp-servers) connecting to Model Client Protocol (MCP) servers. If you've defined [MCP servers in your configuration](#mcp-servers-configuration), then CodeCompanion can automatically connect to those servers when initializing the adapter.
...
<!-- Claude Pro Authorization -->
4. Back in your CLI, copy the OAuth token (in yellow):
<!-- Claude Pro OAuth Token -->
5. In your CodeCompanion config, extend the `claude_code` adapter and include the OAuth token (see the section on [environment variables and setting API keys](#http-adapters-configuration-environment-variables-setting-an-api-key) for other ways to do this):
```lua
require("codecompanion").setup({
  adapters = {
    acp = {
...
> Configure CodeCompanion's chat buffer — keymaps, display options, context management, system prompt, and tool settings for AI-assisted coding in Neovim.
...
See the section on [ACP](#acp-adapters-configuration) and [HTTP](#http-adapters-configuration) for more information.
...
You can also customize the prefixes that trigger completions for [editor context](#editor-context), [slash commands](#slash-commands), and [tools](#agents-and-tools):
...
The `actions` table contains module paths that are resolved and executed asynchronously. See the [generating titles](#using-the-chat-buffer-generating-titles) section for a working example.
...
CodeCompanion makes use of a trigger, a threshold at which the context management begins. You can specify a `trigger` as either a decimal (representing a percentage of the context window) or an integer (representing a token count). When the chat buffer reaches the defined trigger, preventative action is taken by CodeCompanion. You can read more in the [architecture](#architecture-context-management) section.
...
CodeCompanion allows you to apply safety mechanisms to its built-in tools prior to execution. See the [approvals usage](#agents-and-tools-approvals) section for more information.
...
This also works for [extensions](#extensions-configuration).
...
The plugin leverages floating windows to display content to a user in a variety of scenarios, such as with the [debug window](#using-the-chat-buffer-messages). You can change the appearance of the chat buffer by changing the `display.chat.window` table in your configuration.
...
Neovim buffers can be [synced](#editor-context-with-parameters) with the chat buffer. That is, on each turn their content can be shared with the LLM. This is useful if you're modifying a buffer and want the LLM to always have the latest changes.
...
> Configure CLI agents in CodeCompanion — define agents like Claude Code or Codex, set custom commands, configure the terminal provider, and manage input settings.
...
> Configure CodeCompanion extensions to add custom functionality — can be distributed as Neovim plugins or defined locally in your configuration.
...
Visit the creating [extensions](#extending-with-extensions) guide to learn more about available extensions and how to create your own.


---

> Configure CodeCompanion's inline interaction for writing and refactoring code directly into Neovim buffers via LLM prompts, without opening a chat buffer.
...
See the section on [HTTP Adapters](#http-adapters-configuration) for more information.
...
The plugin comes with a number of [editor context](#using-inline-editor-context) items that can be used alongside your prompt using the `#{}` syntax (e.g., `#{my_new_context_item}`). You can also add your own:
...
Please see the [Diff section](#chat-bufferdiff) on the Chat Buffer page for configuration options.


---

> Configure Model Context Protocol (MCP) servers in CodeCompanion to connect Neovim to external tools and data sources via an open AI integration standard.
...
You can find out which parts of the protocol CodeCompanion has implemented on the [MCP](#model-context-protocol-mcp) page. Currently, you can leverage MCP servers with [chat interactions](#using-the-chat-buffer).
...
> Configure CodeCompanion's prompt library with custom Lua or Markdown prompts, reusable workflows, and slash commands for your AI coding workflow in Neovim.
...
In the markdown prompt, above, [placeholders](#prompt-library-configuration-with-placeholders) are used to inject dynamic content from a visual selection.
...
You can also specify [MCP servers](#mcp-servers-configuration) to be loaded with your prompt:
...
**Note:** Markdown prompts do not support [agentic workflows](#extending-with-agentic-workflows).
...
> Configure rules files in CodeCompanion — including CLAUDE.md, AGENTS.md, and Cursor rules — to provide persistent LLM instructions and project context in Neovim.
...
Please see the guide on [Creating Rules Parsers](#extending-with-rules-parsers) to understand how you can create and apply your own.
...
> Customize CodeCompanion's system prompt for chat and inline interactions — replace the default, add dynamic context, or tune language and tone for your LLM.
...
CodeCompanion also ships with a separate system prompt when [tools](#agents-and-tools) are used in the chat buffer:
...
> Configure miscellaneous CodeCompanion options: response language, log level, per-project config files, and restricting code from being sent to LLMs.
...
> Tips and tricks for getting the most out of CodeCompanion in Neovim — keyboard shortcuts, context management, output handling, and productivity patterns.
...
The [@insert_edit_into_file](#agents-and-tools-files) tool, combined with the [#buffer](#editor-context-buffer) editor context or [/buffer](#slash-commands-buffer) slash command, enables an LLM to modify code in a Neovim buffer. This is especially useful if you do not wish to manually apply an LLM's suggestions yourself. Simply tag it in the chat buffer with `@files` or `@insert_edit_into_file`.

## Run tests from the chat buffer

The [run_command](#agents-and-tools-run-command) tool enables an LLM to execute commands on your machine. This can be useful if you wish the LLM to run a test suite on your behalf and give insight on failing cases. Simply tag the `@run_command` in the chat buffer and ask it run your tests.
...
> Use CodeCompanion's Action Palette to launch chat buffers, switch between open chats, access the prompt library, and discover plugin features from a single menu.
...
The plugin also contains two built-in workflows, `Code workflow` and `Edit test repeat workflow`. See the [workflows section](#workflows) for more information.
...
> Everything about CodeCompanion's chat buffer — opening, toggling, keymaps, multi-turn conversations with LLMs, and adding images in Neovim.
...
The backslash trigger is used to avoid conflicts with CodeCompanion's built-in [Slash Commands](#slash-commands). When you send a message, `\command` is automatically transformed to `/command` for the agent. The trigger character can be customized via `interactions.chat.slash_commands.opts.acp.trigger` in your config.
...
Many LLMs have the ability to receive images as input (sometimes referred to as vision). CodeCompanion supports the adding of images into the chat buffer via the [/image](#slash-commands-image) slash command and through the system clipboard with [img-clip.nvim](#installation-img-clip-nvim). CodeCompanion can work with images in your file system and also with remote URLs, encoding both into a base64 representation.
...
> Use CodeCompanion agent tools to let LLMs edit files, run commands, and search the web in Neovim. Covers tool groups, approval system, and model compatibility.
...
An outline of the [tool system architecture](#extending-with-tools-architecture) is available in the extending section.
...
When `system_prompt` is a function, it receives the group config as the first argument and a [context object](#system-prompt-configuration) as the second, giving access to `language`, `date`, `nvim_version`, `os` and more:
...
- [ask_questions](#agents-and-tools-ask-questions)
- [create_file](#agents-and-tools-create-file)
- [delete_file](#agents-and-tools-delete-file)
- [file_search](#agents-and-tools-file-search)
- [get_changed_files](#agents-and-tools-get-changed-files)
- [get_diagnostics](#agents-and-tools-get-diagnostics)
- [grep_search](#agents-and-tools-grep-search)
- [insert_edit_into_file](#agents-and-tools-insert-edit-into-file)
- [read_file](#agents-and-tools-read-file)
- [run_command](#agents-and-tools-run-command)
...
The LLM is specifically instructed to detect if you're running a test suite, and if so, to insert a flag in its request. This is then detected and the outcome of the test is stored in the corresponding flag on the chat buffer. This makes it ideal for [agentic workflows](#extending-with-agentic-workflows) to hook into.
...
The MCP servers you've [configured](#mcp-servers-configuration) in CodeCompanion expose their own set of tools that you can use in the chat buffer. Once a server has been started, the tools will be available to you and appear in the completion menu, by typing `@`. They are prefixed with `mcp:`.
...
> Share Neovim state with your LLM using CodeCompanion editor context — reference buffers, selections, diagnostics, and more with the #{context} syntax in chat.
...
> **TIP:** 
> The [Action Palette](#using-the-action-palette) has a pre-built prompt which asks an LLM to explain LSP diagnostics in a visual selection.
...
> Add rules files like CLAUDE.md, AGENTS.md, or Cursor rules to the CodeCompanion chat buffer to provide persistent LLM instructions and project context.


# Using Rules

Ensure that you have read the [Rules Configuration](#rules-configuration) section to understand how to create and configure rule groups.

## Default Rule Group

Below is the `default` rule group that, when [enabled](#rules-configuration-enabling-rules), provides a collection of common files to the chat buffer:
...
The location of the rules is also unimportant. The rules files could be local to the project you're working in. Or, they could reside in a separate location on your disk. Just ensure the path is correct when you're [creating/configuring](#rules-configuration-rule-groups) the rules group.
...
<!-- Chat buffer action palette -->

There is also a _Chat with rules_ action in the [Action Palette](#using-the-action-palette). This lists all of the rule groups in the config that can be added to a new chat buffer.
...
> Reference for all CodeCompanion slash commands — fetch URLs, add files and buffers, compact message history, insert symbols, and run ACP session options.
...
This slash command is also available in the [CLI prompt input](#using-the-cli-slash-commands), where it inserts `@path` references instead of buffer contents.

## /command

The _command_ slash command is specific to [ACP](#acp-adapters-configuration) adapters and allows users to switch between different adapter commands. For instance, some ACP adapters may allow you to run the agent command with a specific flag. Be mindful that switching commands is destructive and essentially resets the chat buffer for the purposes of a conversation with an agent.
...
This slash command is also available in the [CLI prompt input](#using-the-cli-slash-commands), where it inserts `@path` references instead of file contents.
...
The _rules_ slash command allows you to add [rules](#using-rules) groups to the chat buffer.

## /mcp

The _mcp_ slash command allows you to start and stop [Model Context Protocol (MCP)](#mcp-servers-configuration) servers manually from within a chat buffer. This is applied at a global level, so starting/stopping servers in one chat buffer will affect all other chat buffers. A _snacks.nvim_ and `vim.ui.select` provider is available for selecting which MCP servers to start/stop.

## /mode

The _mode_ slash command is specific to [ACP](#acp-adapters-configuration) adapters and allows users to switch between different agent operating modes, as per the [protocol](https://agentclientprotocol.com/protocol/session-modes) docs.
...
> Interact with CLI agents like Claude Code and Codex from Neovim using CodeCompanion — share context, send prompts, and manage terminals without leaving Neovim.
...
The prompt input buffer also supports the [buffer](#slash-commands-buffer]) and [file](#slash-commands-file) slash commands, which better enable you to share lots of context with a CLI agent at once. Simply type `/` in the buffer to bring up the completion menu for your selected provider.
...
> Reference for all CodeCompanion events and hooks — integrate with Neovim's autocmd system to react to chat, inline, CLI, and tool lifecycle events.
...
> Write and refactor code directly in Neovim buffers using CodeCompanion's inline interaction — supports visual selection, prompt library aliases, and diff review.


# Using Inline


As per the [Getting Started](#getting-startedmdinline) guide, the inline interaction enables you to code directly into a Neovim buffer. Simply run `:CodeCompanion <your prompt>`, or make a visual selection to send that as context to the LLM alongside your prompt.

For convenience, you can call prompts from the [prompt library](#prompt-library-configuration) via the interaction. For example, `:'<,'>CodeCompanion /tests` would ask the LLM to create some unit tests from the selected text.
...
Simply include them in your prompt. For example `:CodeCompanion #{buffer} add a new method to this file`. Multiple context items can be sent as part of the same prompt. You can even add your own custom variables as per the [configuration](#inline-configuration-editor-context).
...
> Use CodeCompanion's prompt library via keymaps, Action Palette, or chat slash commands — includes built-in prompts for explaining, fixing, and testing code.
...
> Run agentic workflows in CodeCompanion — chain multi-step LLM interactions to automate complex coding tasks, launched from the Action Palette.
...
Workflows can only be initiated from the [Action Palette](#using-the-action-palette). This is because they are a complex Lua table structure which needs to be processed and added to a new chat buffer. Simply open up the Action Palette and select your desired workflow.

You can create your own workflows by following the [workflows](#prompt-library-configuration-workflows) configuration guide and the [agentic workflows](#extending-with-agentic-workflows) guide.


---

> Customize CodeCompanion's Neovim UI — configure chat buffer appearance, status line metadata, and use plugin events to build your own interface extensions.
...
CodeCompanion aims to keep any changes to the user's UI to a minimum. Aesthetics, especially in Neovim, are highly subjective. So whilst it won't set much by default, it does endeavour to allow users to hook into the plugin and customize the UI to their liking via [Events](#events-and-hooks).
...
> Build a custom CodeCompanion HTTP adapter to connect Neovim to any LLM. Covers the adapter interface, request handlers, environment variables, and schema.
...
> Build agentic workflows in CodeCompanion — chain LLM prompts with tool calls to automate multi-step tasks like editing files and running your test suite.
...
<details>
<summary>The full `Edit<->Test` workflow code can be found below:</summary>
...
The first prompt in a workflow should set the ask of the LLM and provide clear instructions. In this case, we're giving the LLM access to the [@insert_edit_into_file](#agents-and-tools-files) and [@run_command](#agents-and-tools-run-command) tools to edit a buffer and run tests, respectively.
...
Now there's a little bit more to unpack in this prompt. Firstly, we're automatically submitting the prompt to the LLM to save the user some time and keypresses. Next, we're scoping the prompt to only be sent to the chat buffer if the currently active tool is the [@run_command](#agents-and-tools-run-command).

We're also leveraging a function called `repeat_until`. This ensures that the prompt is always attached to the chat buffer until a condition is met. In this case, until the tests pass. In the [@run_command](#agents-and-tools-run-command) tool, we ask the LLM to pass a flag if it detects a test suite is being run. The plugin picks up on that flag and puts the test outcome into the chat buffer class as a flag.
...
> Create a CodeCompanion extension to add custom functionality to the plugin — distributable as a Neovim plugin or defined locally in your configuration.
...
> Create custom rules parsers in CodeCompanion to post-process and transform rules file content before it's shared with an LLM.
...
> Build custom CodeCompanion tools to let LLMs execute functions in Neovim — covers tool structure, OpenAI-compatible schemas, handlers, and agent group integration.
...
1. Chat Buffer sends prompt with tool schemas to the LLM
2. LLM responds with tool call(s)
3. Chat Buffer -> Tool System: Parse LLM response for tool calls
4. For each tool call: resolve tool config, add to execution queue
5. Tool System -> Orchestrator: Create orchestrator with the queue
6. For each tool in queue:
   a. Pop tool, set up handlers and output functions
   b. If approval required -> prompt user -> handle accept/reject/cancel
   c. Execute each command in tool.cmds (sync or async via output_cb)
   d. Call output.success() or output.error() per command
   e. tool:add_tool_output() to update chat buffer
7. Call handlers.on_exit() when all commands finish
8. Reset tool system, fire completion autocmd
...
1. `setup` - Is called **before** anything in the [cmds](#extending-with-tools-cmds) and [output](#extending-with-tools-output) table. This is useful if you wish to set the cmds dynamically on the tool itself, like in the [@run_command](https://github.com/olimorris/codecompanion.nvim/blob/main/lua/codecompanion/interactions/chat/tools/builtin/run_command.lua) tool.
2. `on_exit` - Is called **after** everything in the [cmds](#extending-with-tools-cmds) and [output](#extending-with-tools-output) table.
3. `prompt_condition` - Is called **before** anything in the [cmds](#extending-with-tools-cmds) and [output](#extending-with-tools-output) table and is used to determine _if_ the user should be prompted for approval. This is used in the `@insert_edit_into_file` tool to allow users to determine if they'd like to apply an approval to _buffer_ or _file_ edits.
...
The _output_ table enables you to manage and format output from the execution of the [cmds](#extending-with-tools-cmds). It contains four functions:
...
2. `error` - Is called when an error occurs whilst executing a command/function. It will only ever be called once as the whole execution of the [cmds](#extending-with-tools-cmds) is halted. This can be a useful way of notifying the LLM of the failure.
3. `prompt` - Is called when user approval to execute the [cmds](#extending-with-tools-cmds) is required. It forms the message prompt which the user is asked to confirm or reject.
4. `rejected` - Is called when a user rejects the approval to run the [cmds](#extending-with-tools-cmds). This method is used to inform the LLM of the rejection.
...
For a lot of users, custom tools will often be commands that they ask an LLM to execute on their machine. As such, the handlers and output functions that exist in the [run_command](#agents-and-tools-run-command) tool are sufficient and should be reused.
...
> Examples and community recipes for extending CodeCompanion's UI in Neovim — including progress spinners with Fidget.nvim and custom status line integrations.
