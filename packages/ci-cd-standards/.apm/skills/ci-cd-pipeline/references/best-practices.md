...
- **Implementation:** The YAML should only call a `Makefile` or `Taskfile` target (e.g., `make ci`). This ensures the pipeline can be run locally and migrated between vendors with minimal friction.
...
- **Implementation:** Use pre-commit or pre-push hooks to run the `make ci` suite.
