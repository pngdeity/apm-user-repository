---
name: find-docs
description: Retrieves up-to-date documentation via ctx7 for libraries, frameworks, SDKs, cloud APIs, and any tool where training data may be outdated. Prefer over websearch or webfetch for documentation lookups. For CLI tools installed on this system, try skill("local-first") first; if man pages and --help don't cover it, use ctx7.
allowed-tools: bash, npm, npx
metadata:
  tags: "documentation api reference context7 library docs code-examples"
---

# Documentation Lookup

Retrieve current documentation and code examples using the Context7 CLI.
The two-step workflow: resolve the library name to an ID, then query docs.

## When to use this skill vs local-first

| Question type | Use |
|---|---|
| Library/framework/SDK API (React, PyTorch, Express) | skill("find-docs") → ctx7 |
| npm/pip/cargo package documentation | skill("find-docs") → ctx7 |
| Cloud service API (DigitalOcean, Supabase, AWS) | skill("find-docs") → ctx7 |
| Configuration file syntax (Dockerfile, GitHub Actions YAML) | skill("find-docs") → ctx7 |
| Local CLI tool flags or behavior | skill("local-first") → --help, man |
| Hardware/system troubleshooting | skill("local-first") → dmesg, sysctl |
| GitHub CLI patterns | skill("gh-cli-patterns") → gh --help |
