---
name: use-modern-go
description: Apply modern Go syntax guidelines based on the project's Go version. Use when writing Go code or when asked for modern Go code guidelines.
allowed-tools: bash
metadata:
  tags: "go modern guidelines slices maps errors context sync testing"
compatibility: Works with any Go project that has a go.mod file
---
...
To detect the Go version, run:

```bash
grep -rh "^go " --include="go.mod" . 2>/dev/null | cut -d' ' -f2 | sort | uniq -c | sort -nr | head -1 | xargs | cut -d' ' -f2 | grep . || echo unknown
```

DO NOT search for go.mod files manually or try to detect the version yourself. Use ONLY the version from the command above.
...
- Use the question tool to ask: "Which Go version should I target?" → [1.23] / [1.24] / [1.25] / [1.26]
